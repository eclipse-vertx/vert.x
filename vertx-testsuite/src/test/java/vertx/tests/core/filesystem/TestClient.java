...
public class TestClient extends TestClientBase {
	...
	private void testCopy(final String source, final String target, final boolean recursive, final boolean shouldPass,
			final Handler<Void> afterOK) {
	...
	private void testMove(final String source, final String target, final boolean shouldPass, final Handler<Void> afterOK)
			throws Exception {
		vertx.fileSystem().move(TEST_DIR + pathSep + source, TEST_DIR + pathSep + target,
	...
	private void azzertPerms(final String perms, final String file1) {
	...
	private void testChmod(final String file, final String perms, final String dirPerms, final boolean shouldPass,
			final Handler<Void> afterOK) throws Exception {
		if (Windows.isWindows() == false) {
	...
	private void testLink(final String from, final String to, final boolean symbolic, final boolean shouldPass,
			final Handler<Void> afterOK) throws Exception {
		AsyncResultHandler<Void> handler = createHandler(shouldPass, afterOK);
		if (symbolic) {
			// Symlink is relative
	...
	public void testMkdirSimple() throws Exception {
		...
		testMkdir(dirName, null, false, true, new SimpleHandler() {
			...
			public void handle() {
				...
				azzertPerms(DEFAULT_DIR_PERMS, dirName);
			}
		});
	}
	...
	public void testWriteAsync() throws Exception {
		...
		vertx.fileSystem().open(TEST_DIR + pathSep + fileName, null, false, true, true, true,
	...
	public void testReadAsync() throws Exception {
		...
		createFile(fileName, content);
		vertx.fileSystem().open(TEST_DIR + pathSep + fileName, null, true, false, false,
				new AsyncResultHandler<AsyncFile>() {
					int reads;
	...
	public void testPumpFileStreams() throws Exception {
		...
		vertx.fileSystem().open(TEST_DIR + pathSep + fileName1, null, true, false, false,
				new AsyncResultHandler<AsyncFile>() {
					@Override
					public void handle(final AsyncResult<AsyncFile> arr) {
						tu.checkContext();
						if (arr.exception == null) {
							...
							vertx.fileSystem().open(TEST_DIR + pathSep + fileName2, null, true, true, true,
									new AsyncResultHandler<AsyncFile>() {
	...
	private void testFSProps(final String fileName, final Handler<FileSystemProps> afterOK) throws Exception {
