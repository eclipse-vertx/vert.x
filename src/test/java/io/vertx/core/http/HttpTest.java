...
public abstract class HttpTest extends HttpTestBase {
  ...
  public void testPauseClientResponse() {
    ...
    HttpClientRequest clientRequest = client.request(HttpMethod.GET, DEFAULT_HTTP_PORT, DEFAULT_HTTP_HOST, DEFAULT_TEST_URI, onSuccess(resp -> {
      resp.pause();
      paused.set(true);
      resp.handler(chunk -> {
        if (paused.get()) {
  ...
  private void testDeliverPausedBufferWhenResume(Consumer<Runnable> scheduler) throws Exception {
    ...
    for (int i = 0;i < num;i++) {
      int idx = i;
      client.request(HttpMethod.GET, DEFAULT_HTTP_PORT, DEFAULT_HTTP_HOST, "/" + i, onSuccess(resp -> {
        Buffer body = Buffer.buffer();
  ...
  public void testEventHandlersNotHoldingLock() throws Exception {
    ...
    startServer();
    for (int i = 0;i < 2;i++) {
      client.post(DEFAULT_HTTP_PORT, DEFAULT_HTTP_HOST, "/" + i, onSuccess(resp -> {
        assertEquals(200, resp.statusCode());
  ...
  public void testEventHandlersNotHoldingLockOnClose() throws Exception {
    ...
    startServer();
    HttpClientRequest req = client.get(DEFAULT_HTTP_PORT, DEFAULT_HTTP_HOST, "/somepath", onSuccess(resp -> {
      assertEquals(200, resp.statusCode());
      HttpConnection conn = resp.request().connection();
      resp.exceptionHandler(err -> {
        assertFalse(Thread.holdsLock(conn));
