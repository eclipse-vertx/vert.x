...
public class Http2Test extends HttpTest {
  ...
  public void testServerStreamPausedWhenConnectionIsPaused() throws Exception {
    ...
    startServer(testAddress);
    client.get(new RequestOptions(requestOptions).setURI("/0"))
      .onComplete(onSuccess(resp -> {
        resp.pause();
    ...
    client.get(new RequestOptions(requestOptions).setURI("/1"))
      .onComplete(onSuccess(resp -> {
        resp.endHandler(v -> {
          complete();
        });
  ...
  public void testInitialMaxConcurrentStreamZero() throws Exception {
    ...
    client.send(new RequestOptions(requestOptions).setTimeout(10000))
      .onComplete(resp -> {
        complete();
      });
    await();
