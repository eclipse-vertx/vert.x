...
public abstract class HttpTracingTestBase extends HttpTestBase {
  ...
  public void testMultipleHttpServerRequest() throws Exception {
    ...
    for (Span server2Span: lastServerSpans) {
      ...
      assertEquals(scheme + "://localhost:8080/2?id=" + count, client2Span.getTags().get("http.url"));
      ...
      assertEquals(scheme + "://localhost:8080/1?id=" + count, server1Span.getTags().get("http.url"));
      assertEquals("200", client2Span.getTags().get("http.status_code"));
      ...
      assertEquals(scheme + "://localhost:8080/1?id=" + count, client1Span.getTags().get("http.url"));
      assertEquals("200", client2Span.getTags().get("http.status_code"));
