 ...
 */
public class DeploymentTest2 extends VertxTestBase {
  ...
  @Repeat(times=1000)
  @Test
  public void undeployChildWhileAlreadyUndeploying() throws InterruptedException, ExecutionException, TimeoutException {
    ...
    vertx.deployVerticle(parent, id -> vertx.undeploy(id.result(), res -> {
  ...
  @Test
  public void undeployAChildMakesDeploymentHalt() throws InterruptedException, ExecutionException, TimeoutException {

    Vertx v = Vertx.vertx();
    CompletableFuture<AsyncResult<String>> childResult = new CompletableFuture<>();
    CompletableFuture<AsyncResult<String>> parentResult = new CompletableFuture<>();
    CompletableFuture<AsyncResult<Void>> undeployChild = new CompletableFuture<>();
    CompletableFuture<AsyncResult<Void>> undeployParent = new CompletableFuture<>();

    Verticle parent = new AbstractVerticle() {
      @Override
      public void start() throws Exception {
        this.vertx.deployVerticle(new AbstractVerticle() {}, id -> {
          childResult.complete(id);
        });
      }
    };


    v.deployVerticle(parent, id -> parentResult.complete(id));

    AsyncResult<String> stringAsyncResult = childResult.get();

    assertTrue(stringAsyncResult.succeeded());

    v.undeploy(stringAsyncResult.result(), result -> undeployChild.complete(result));
    assertTrue(undeployChild.get()
      .succeeded());
    v.undeploy(parentResult.get()
      .result(), result -> undeployParent.complete(result));

    assertTrue(undeployParent.get(1, TimeUnit.SECONDS)
      .succeeded());
  }
