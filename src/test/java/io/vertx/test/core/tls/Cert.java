...
public interface Cert<K extends KeyCertOptions> extends Supplier<K> {
  ...
  Cert<PemKeyCertOptions> SERVER_VIRT_PEM = () -> new PemKeyCertOptions().setKeyPath("tls/server-virt-key.pem").setCertPath("tls/server-virt-cert.pem");
  Cert<JksOptions> SERVER_VIRT_JKS = () -> new JksOptions().setPath("tls/server-virt-keystore.jks").setPassword("wibble");
  Cert<PfxOptions> SERVER_VIRT_PKCS12 = () -> new PfxOptions().setPath("tls/server-virt-keystore.p12").setPassword("wibble");
