...
public class TestUtils {
  ...
  public static Buffer leftPad(int padding, Buffer buffer) {
    try (BufferAllocator alloc = BufferAllocator.onHeapUnpooled()) {
      io.netty5.buffer.api.Buffer internal = alloc.allocate(padding + buffer.length());
      internal.readerOffset(padding);
      internal.writerOffset(padding);
      Buffer ret = Buffer.buffer(internal);
      ret.appendBuffer(buffer);
      return ret;
    }
  }
