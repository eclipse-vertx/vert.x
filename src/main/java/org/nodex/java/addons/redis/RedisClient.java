...
public class RedisClient {
  ...
  private final ConnectionPool<RedisConnection> pool = new ConnectionPool<RedisConnection>() {
    protected void connect(Handler<RedisConnection> connectHandler, long contextID) {
   ...
   */
  public RedisClient setPort(int port) {
    this.port = port;
    return this;
  }
  ...
  private void sendRequest(final RedisDeferred<?> deferred, final Buffer request) {
    pool.getConnection(new Handler<RedisConnection>() {
      public void handle(RedisConnection conn) {
        conn.writeRequest(deferred, request);
  ...
  private void internalConnect(final Handler<RedisConnection> connectHandler, long contextID) {
