...
public class Http2ServerRequestImpl extends VertxHttp2Stream<Http2ServerConnection> implements HttpServerRequest {
  ...
  void dispatch(Handler<HttpServerRequest> handler) {
    ...
    if (tracer != null) {
      ...
      trace = tracer.receiveRequest(context.localContextData(), this, method().name(), headers(), tags);
    }
  ...
  void handleClose() {
    ...
    if (tracer != null) {
      ...
      tracer.sendResponse(context.localContextData(), failure == null ? response : null, trace, failure, tags);
    }
    if (handler != null) {
