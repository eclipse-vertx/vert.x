  ...
  static class Http2ClientStream extends VertxHttp2Stream<Http2ClientConnection> implements HttpClientStream {
    ...
    void handleClose() {
      ...
      if (tracer != null) {
        ...
        tracer.receiveResponse(context.localContextData(), failure == null ? response : null, trace, failure, failure == null ? Collections.singletonList(new AbstractMap.SimpleEntry<>("http.status_code", "" + response.statusCode())) : null);
      }
    ...
    public void beginRequest(HttpClientRequestImpl req) {
      ...
      if (tracer != null) {
        ...
        trace = tracer.sendRequest(context.localContextData(), request, request.method.name(), headers, tags);
      }
    }
