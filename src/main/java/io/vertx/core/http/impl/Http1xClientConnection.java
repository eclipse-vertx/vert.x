    ...
    public void beginRequest(HttpClientRequestImpl req) {
      synchronized (conn) {
        ...
        if (tracer != null) {
          ...
          trace = tracer.sendRequest(request.stream.getContext().localContextData(), request, request.method.name(), headers, tags);
        }
      }
    }
    ...
    private boolean endResponse(LastHttpContent trailer) {
      synchronized (conn) {
        ...
        if (tracer != null) {
          ...
          tracer.receiveResponse(context.localContextData(), response, trace, null, tags);
        }
  ...
  protected void handleClosed() {
    ...
    synchronized (this) {
      ...
      for (StreamImpl r = responseInProgress;r != null;r = r.next) {
        ...
        if (tracer != null) {
          tracer.receiveResponse(r.context.localContextData(), null, r.trace, ConnectionBase.CLOSED_EXCEPTION, Collections.emptyList());
        }
        if (list.isEmpty()) {
          list = new ArrayList<>();
        }
