...
public interface Vertx extends Measured {
  ...
  /**
   * Safely execute some blocking code in the given pool.
   * <p>
   * Executes the blocking code in the handler {@code blockingCodeHandler} using a thread from the given pool. The
   * pool must have been defined using the {@link io.vertx.core.spi.NamedThreadPoolFactory} SPI. If not configured, the
   * worker pool is used.
   * <p>
   * When the code is complete the handler {@code resultHandler} will be called with the result on the original context
   * (e.g. on the original event loop of the caller).
   ...
   * @param <T> the type of the result
   */
  <T> void executeBlocking(Handler<Future<T>> blockingCodeHandler, boolean ordered, Handler<AsyncResult<T>>
      resultHandler, String poolName);
  ...
  /**
   * Like {@link #executeBlocking(Handler, boolean, Handler, String)} called with ordered = true.
   */
  <T> void executeBlocking(Handler<Future<T>> blockingCodeHandler, Handler<AsyncResult<T>> resultHandler,
                           String poolName);
