...
public class EventBusImpl implements EventBus, MetricsProvider {
  ...
  protected final Object messageSent(OutboundDeliveryContext<?> sendContext, boolean local, boolean remote) {
    ...
    Tracer tracer = vertx.tracer();
    if (tracer != null && sendContext.message.src) {
      ConcurrentMap<Object, Object> data = sendContext.ctx.localContextData();
      BiConsumer<String, String> biConsumer = (String key, String val) -> sendContext.message.headers().set(key, val);
      List<Map.Entry<String, String>> tags = Collections.singletonList(new AbstractMap.SimpleEntry<>("peer.service", sendContext.message.address));
      return tracer.sendRequest(data, sendContext.message, "send", biConsumer, tags);
    } else {
      return null;
    }
  }
  ...
  private <T> void sendLocally(OutboundDeliveryContext<T> sendContext) {
    ...
    if (!deliverMessageLocally(sendContext.message)) {
      ...
      if (sendContext.replyHandler != null) {
        ...
        if (tracer != null && sendContext.message.src) {
          tracer.receiveResponse(sendContext.ctx.localContextData(), null, trace, failure, Collections.emptyList());
        }
      }
    } else {
      Tracer tracer = vertx.tracer();
      if (tracer != null && sendContext.message.src) {
        if (sendContext.replyHandler == null) {
          tracer.receiveResponse(sendContext.ctx.localContextData(), null, trace, null, Collections.emptyList());
        } else {
          sendContext.replyHandler.trace = trace;
        }
      }
  ...
  public class ReplyHandler<T> implements Handler<Message<T>> {
    ...
    private void trace(Object reply, Throwable failure) {
      ...
      if (tracer != null && registration.src) {
        tracer.receiveResponse(ctx.localContextData(), reply, trace, failure, Collections.emptyList());
      }
    }
