...
public class HandlerRegistration<T> implements MessageConsumer<T>, Handler<Message<T>> {
  ...
  protected class InboundDeliveryContext implements DeliveryContext<T> {
    ...
    private InboundDeliveryContext(MessageImpl<?, T> message, Handler<Message<T>> handler, ContextInternal context) {
      ...
      this.context = message.src ? context : context.createTraceContext();
    }
    ...
    public void next() {
      if (iter.hasNext()) {
        ...
        try {
          ...
          Tracer tracer = handlerContext.owner().tracer();
          if (tracer != null && !src) {
            List<Map.Entry<String, String>> tags = Collections.singletonList(new AbstractMap.SimpleEntry<>("peer.service", message.address));
            Object trace = tracer.receiveRequest(context.localContextData(), message, "send", message.headers, tags);
            handler.handle(message);
            tracer.sendResponse(context.localContextData(), null, trace, null, Collections.emptyList());
          } else {
            handler.handle(message);
          }
          if (metrics != null) {
