...
public class DefaultFileSystem implements FileSystem {
	...
	protected final VertxInternal getVertx() {
		return vertx;
	}
	...
	@Override
	public void open(String path, String perms, boolean read, boolean write, boolean createNew, boolean flush,
			AsyncResultHandler<AsyncFile> handler) {
	...
	private BlockingAction<Void> copyInternal(String from, String to, final boolean recursive,
		...
		return new BlockingAction<Void>(vertx, handler) {
			...
			public Void action() throws Exception {
				try {
					...
					throw new FileSystemException("Failed to move between " + source + " and " + target
			...
			public Void action() throws Exception {
				...
				try {
					...
					throw new FileSystemException("Cannot open file " + path
							+ ". Either it is a directory or you don't have permission to change it");
				} finally {
					if (raf != null)
						raf.close();
				}
				...
				try {
					if (dirPermissions != null) {
						Files.walkFileTree(target, new SimpleFileVisitor<Path>() {
							@Override
							public FileVisitResult preVisitDirectory(Path dir, BasicFileAttributes attrs) throws IOException {
								// The directory entries typically have
								// different permissions to the files,
								// e.g. execute permission
								// or can't cd into it
	...
	private BlockingAction<Void> link(String link, String existing, final boolean symbolic,
			AsyncResultHandler<Void> handler) {
			...
			public Void action() throws Exception {
				if (recursive) {
					...
					try {
						...
						throw new FileSystemException("Cannot delete directory, it is not empty: " + source
	...
	protected BlockingAction<Void> mkdirInternal(String path, final String perms, final boolean createParents,
			AsyncResultHandler<Void> handler) {
		final Path source = PathAdjuster.adjust(Paths.get(path));
		final FileAttribute<?> attrs = (perms == null ? null : PosixFilePermissions.asFileAttribute(PosixFilePermissions
				.fromString(perms)));
		return new BlockingAction<Void>(vertx, handler) {
			@Override
			public Void action() throws Exception {
	...
	private BlockingAction<AsyncFile> openInternal(String path, String perms, boolean createNew,
			AsyncResultHandler<AsyncFile> handler) {
		return openInternal(path, perms, true, true, createNew, false, handler);
	}

	private BlockingAction<AsyncFile> openInternal(String path, String perms, boolean read, boolean write,
			boolean createNew, AsyncResultHandler<AsyncFile> handler) {
		return openInternal(path, perms, read, write, createNew, false, handler);
	}

	private BlockingAction<AsyncFile> openInternal(String p, final String perms, final boolean read, final boolean write,
			final boolean createNew, final boolean flush, AsyncResultHandler<AsyncFile> handler) {
		final String path = PathAdjuster.adjust(p);
