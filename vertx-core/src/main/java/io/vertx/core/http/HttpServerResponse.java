 ...
 * {@link java.nio.channels.Channel} and manage its lifecycle use {@link #sendFile(Channel, String)}.
...
public interface HttpServerResponse extends WriteStream<Buffer> {
  ...
  /**
   * Same as {@link #sendFile(Channel, String, long)} using length @code{Long.MAX_VALUE} which means until the end of the
   * file.
   *
   ...
   */
  @GenIgnore(GenIgnore.PERMITTED_TYPE)
  default Future<Void> sendFile(Channel channel, String extension) {
    return sendFile(channel, extension, 0);
  }

  /**
   * Same as {@link #sendFile(Channel, String, long, long)} using length @code{Long.MAX_VALUE} which means until the end of the
   * file.
   *
   * @param channel the file channel to the file to serve
   * @param extension the file extension if known
   ...
   */
  @GenIgnore(GenIgnore.PERMITTED_TYPE)
  default Future<Void> sendFile(Channel channel, String extension, long offset) {
   ...
   */
  @GenIgnore(GenIgnore.PERMITTED_TYPE)
  Future<Void> sendFile(Channel channel, String extension, long offset, long length);
