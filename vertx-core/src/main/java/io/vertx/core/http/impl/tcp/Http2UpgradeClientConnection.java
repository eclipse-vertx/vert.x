...
public class Http2UpgradeClientConnection implements io.vertx.core.http.impl.HttpClientConnection {
  ...
  public Http2UpgradeClientConnection(Http1ClientConnection connection, ClientMetrics<?, ?, ?> metrics, Http2ChannelUpgrade upgrade) {
  ...
  private static class UpgradingStream implements HttpClientStream {
    ...
    UpgradingStream(HttpClientStream stream, Http2UpgradeClientConnection upgradedConnection, ClientMetrics<?, ?, ?> metrics, Http2ChannelUpgrade upgrade, Http1ClientConnection upgradingConnection) {
  ...
  public Future<HttpClientStream> createStream(ContextInternal context) {
    if (current instanceof Http1ClientConnection && !upgradeProcessed) {
      return current
        ...
        .map(stream -> new UpgradingStream(stream, this, metrics, upgrade, (Http1ClientConnection) current));
    } else {
      return current
        .createStream(context)
