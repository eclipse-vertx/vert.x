...
public class Http3ChannelConnector implements HttpChannelConnector {
  ...
  public Http3ChannelConnector(VertxInternal vertxInternal, HttpClientMetrics<?, ?, ?> clientMetrics,  Http3ClientOptions options) {
  ...
  public Future<HttpClientConnection> httpConnect(ContextInternal context, SocketAddress server, HostAndPort authority, HttpConnectParams params, long maxLifetimeMillis, ClientMetrics<?, ?, ?> metrics) {
    ...
    if (fut == null) {
      ...
      QuicClient client = new QuicClientImpl(vertx, metricsProvider, options);
    ...
    fut.onComplete((res, err) -> {
      if (err == null) {
        ...
        f.onComplete((res2, err2) -> {
          if (err2 == null) {
            Http3ClientConnection c = new Http3ClientConnection((QuicConnectionInternal) res2, authority, (ClientMetrics<Object, HttpRequest, HttpResponse>) metrics);
            c.init();
            promise.complete(c);
          } else {
            promise.fail(err2);
