...
public class NetServerImpl implements NetServerInternal {
  ...
  private class NetSocketInitializer {
    ...
    private void connected(Channel ch, SslContextManager sslContextManager, SSLOptions sslOptions) {
      ...
      VertxHandler<NetSocketImpl> handler = VertxHandler.create(ctx -> new NetSocketImpl(context, ctx, null, sslContextManager, sslOptions, metrics, registerWriteHandler, config.getSendFileChunkSize()));
