 ...
 */
@VertxGen
public interface TcpClient extends Measured {
   ...
   * {@link TcpSocket} instance is supplied via the {@code connectHandler} instance
   *
   * @param port  the port
   * @param host  the host
   * @return a future notified when the socket is connected
   */
  Future<TcpSocket> connect(int port, String host);
   ...
   * @return a future notified when the socket is connected
   */
  Future<TcpSocket> connect(int port, String host, String serverName);

  /**
   * Open a connection to a server at the specific {@code remoteAddress}.
   * <p>
   * The connect is done asynchronously and on success, a {@link TcpSocket} instance is supplied via the {@code connectHandler} instance
   *
   * @param remoteAddress the remote address
   * @return a future notified when the socket is connected
   */
  Future<TcpSocket> connect(SocketAddress remoteAddress);
   ...
   * @return a future notified when the socket is connected
   */
  Future<TcpSocket> connect(SocketAddress remoteAddress, String serverName);
   ...
   * @return a future notified when the socket is connected
   */
  Future<TcpSocket> connect(ConnectOptions connectOptions);

  /**
   * Close immediately ({@code shutdown(Duration.ZERO}).
   *
