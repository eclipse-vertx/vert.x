...
public class ProxyFactory {
  ...
  public <T> Registration registerService(T service, String address) {
    return eventBus.registerHandler(address, (Message<JsonObject> msg) -> {
      String op = msg.body().getString(OPS_FIELD);
      JsonArray args = msg.body().getArray(ARGS_FIELD);
  ...
  private class ProxyInvocationHandler implements InvocationHandler {
    ...
    public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
      ...
      eventBus.sendWithTimeout(address, msg, proxyOperationTimeout, (AsyncResult<Message<Object>> ar) -> {
        if (fHandler != null) {
          if (ar.failed()) {
            fHandler.handle(ar);
          } else {
