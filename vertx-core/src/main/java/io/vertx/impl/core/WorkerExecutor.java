...
public class WorkerExecutor implements EventExecutor {
  ...
  public static io.vertx.impl.core.WorkerExecutor unwrapWorkerExecutor() {
    ContextInternal ctx = VertxImpl.currentContext();
    if (ctx != null) {
      ctx = ctx.unwrap();
      Executor executor = ctx.executor();
      if (executor instanceof io.vertx.impl.core.WorkerExecutor) {
        return (io.vertx.impl.core.WorkerExecutor) executor;
      } else {
