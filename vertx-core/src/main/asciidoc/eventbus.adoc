...
When Vert.x has tracing enabled, the tracing behavior of event bus messaging can be controlled independently on both
the producer and consumer sides with a {@link io.vertx.core.tracing.TracingPolicy}:

- {@link io.vertx.core.tracing.TracingPolicy#PROPAGATE}: propagate an existing trace (default)
- {@link io.vertx.core.tracing.TracingPolicy#ALWAYS}: propagate an existing trace or create a new trace when none exists
- {@link io.vertx.core.tracing.TracingPolicy#IGNORE}: disable tracing

The *producer* tracing policy is set on {@link io.vertx.core.eventbus.DeliveryOptions} when sending or publishing a message.
It controls whether the producer creates or propagates a trace span.

[source,$lang]
----
{@link examples.EventBusExamples#example15}
----

The *consumer* tracing policy is set on {@link io.vertx.core.eventbus.MessageConsumerOptions} when registering a consumer.
It controls whether the consumer creates a receive span for incoming messages.

[source,$lang]
----
{@link examples.EventBusExamples#example16}
----
