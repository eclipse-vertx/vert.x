...
`JsonObject` / `JsonArray` encoding and decoding is backed by the Jackson library, {@link io.vertx.core.json.Json} static
methods uses it as well.

[#_supported_jackson_versions]
==== Supported Jackson versions

Vert.x depends on the https://github.com/FasterXML/jackson/wiki/Jackson-Releases[latest available LTS version] of Jackson 2: 2.21.x

Since Vert.x 5.1, Jackson 3 is supported and utilized when Jackson 2 is not available on the class/module path, the Jackson
3 version shall be the latest available LTS version of Jackson 3: 3.1.x

[IMPORTANT]
====
Vert.x has a transitive dependency on `com.fasterxml.jackson.core:jackson-core` (Jackson 2).
When using Jackson 3 (`tools.jackson.core`), you must explicitly exclude that transitive dependency from `vertx-core` to avoid having both Jackson 2 and Jackson 3 on the classpath (or module path) simultaneously.
...
implementation 'tools.jackson.core:jackson-core:3.1.x'
----
====
...
Jackson Databind extension is supported, powering JSON/Object mapping facilities such as {@link io.vertx.core.json.JsonObject#mapTo(java.lang.Class)} or {@link io.vertx.core.json.Json#encode(java.lang.Object)}.

By choice, Jackson Databind is not a dependency of `io.vertx:vertx-core`, it should be explicitly added as a dependency of your project.

[IMPORTANT]
====
When using Jackson 3, adding the Jackson Databind dependency is not enough.
You also have to exclude `com.fasterxml.jackson.core:jackson-core` (Jackson 2) as explained in <<_supported_jackson_versions>>.
====
