...
When using Jackson 3 (`tools.jackson.core`), you must explicitly exclude that transitive dependency from `vertx-core` to avoid having both Jackson 2 and Jackson 3 on the classpath simultaneously.
