...
public abstract class HttpTCPTLSTest extends HttpTLSTest {
  ...
  private void testUpdateSSLOptions(Function<Integer, JksOptions> blah, Function<Integer, JksOptions> bluh, boolean force, boolean updateTrust) throws Exception {
    server = vertx.createHttpServer(createBaseServerOptions().setSsl(true).setKeyCertOptions(blah.apply(0)))
      .requestHandler(req -> {
    ...
    HttpClient client1 = vertx.createHttpClient(createBaseClientOptions().setSsl(true).setVerifyHost(false).setTrustOptions(bluh.apply(0)));
    HttpClientAgent client2 = vertx.createHttpClient(createBaseClientOptions().setSsl(true).setVerifyHost(false).setTrustOptions(bluh.apply(0)));
  ...
  public void testUpdateWithInvalidSSLOptions() throws Exception {
    ...
    startServer(testAddress);
    client = vertx.createHttpClient(createBaseClientOptions().setSsl(true).setVerifyHost(false).setTrustOptions(Trust.SERVER_JKS.get()));
  ...
  public void testServerSharingUpdateSSLOptions() throws Exception {
    ...
    for (int i = 0;i < num;i++) {
      ...
      servers[i] = vertx.createHttpServer(createBaseServerOptions()
          .setSsl(true)
          .setKeyCertOptions(Cert.SERVER_JKS.get()))
        .requestHandler(req -> {
    ...
    for (int i = 0;i < num;i++) {
      clients[i] = vertx.createHttpClient(createBaseClientOptions().setSsl(true).setVerifyHost(false).setTrustOptions(Trust.SERVER_JKS.get()));
    }
    for (int i = 0;i < num;i++) {
