...
public class SslContextManagerTest extends VertxTestBase {
  ...
  public void testUseJdkCiphersWhenNotSpecified() throws Exception {
    ...
    helper
      .buildSslContextProvider(new SSLOptions().setKeyCertOptions(Cert.CLIENT_JKS.get()).setTrustOptions(Trust.SERVER_JKS.get()), null, ClientAuth.NONE, new ArrayList<>(), null, false, (ContextInternal) vertx.getOrCreateContext())
  ...
  public void testUseOpenSSLCiphersWhenNotSpecified() throws Exception {
    ...
    helper.buildSslContextProvider(new SSLOptions().setKeyCertOptions(Cert.CLIENT_PEM.get()).setTrustOptions(Trust.SERVER_PEM.get()), null, ClientAuth.NONE, new ArrayList<>(), null, false, (ContextInternal) vertx.getOrCreateContext()).onComplete(onSuccess(provider -> {
      SslContext ctx = provider.createContext(false, false);
  ...
  private void testOpenSslServerSessionContext(boolean testDefault){
    ...
    defaultHelper
      .buildSslContextProvider(sslOptions, null, ClientAuth.NONE, new ArrayList<>(), null, false, (ContextInternal) vertx.getOrCreateContext())
      .onComplete(onSuccess(provider -> {
  ...
  public void testPreserveEnabledCipherSuitesOrder() throws Exception {
    ...
    SslContextManager helper = new SslContextManager(SslContextManager.resolveEngineOptions(null, false));
    helper
      .buildSslContextProvider(options, null, ClientAuth.NONE, new ArrayList<>(), null, false, (ContextInternal) vertx.getOrCreateContext())
  ...
  public void testCache() throws Exception {
    ...
    SslContextProvider f1 = awaitFuture(helper.resolveSslContextProvider(options, "", ClientAuth.NONE, new ArrayList<>(), null, ctx));
    SslContextProvider f2 = awaitFuture(helper.resolveSslContextProvider(options, "", ClientAuth.NONE, new ArrayList<>(), null, ctx));
    assertSame(f1, f2);
    awaitFuture(helper.resolveSslContextProvider(new SSLOptions().setKeyCertOptions(Cert.SERVER_PKCS12.get()), "", ClientAuth.NONE, new ArrayList<>(), null, ctx));
    awaitFuture(helper.resolveSslContextProvider(new SSLOptions().setKeyCertOptions(Cert.SERVER_PEM.get()), "", ClientAuth.NONE, new ArrayList<>(), null, ctx));
    awaitFuture(helper.resolveSslContextProvider(new SSLOptions().setKeyCertOptions(Cert.CLIENT_PEM.get()), "", ClientAuth.NONE, new ArrayList<>(), null, ctx));
    awaitFuture(helper.resolveSslContextProvider(new SSLOptions().setKeyCertOptions(Cert.SNI_PEM.get()), "", ClientAuth.NONE, new ArrayList<>(), null, ctx));
    f2 = awaitFuture(helper.resolveSslContextProvider(options, "", ClientAuth.NONE, new ArrayList<>(), null, ctx));
    assertNotSame(f1, f2);
  }
