...
//        assertEquals(2, seq.get());
  ...
  public void testHttpClientRequest() throws Exception {
    testHttpClientRequest(new RequestOptions().setPort(config.port()).setHost(config.host()).setURI("/"), "GET");
  }

  @Test
  public void testHttpClientRequestOverrideOperation() throws Exception {
    testHttpClientRequest(new RequestOptions().setPort(config.port()).setHost(config.host()).setURI("/").setTraceOperation("operation-override"), "operation-override");
  }
