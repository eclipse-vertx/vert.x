...
//    }
  ...
  public static class Client {
    ...
    public Connection connect(InetSocketAddress server) throws Exception {
      ...
      AtomicReference<Runnable> goAwayHandlerRef = new AtomicReference<>();
          ...
          public void channelRead(ChannelHandlerContext ctx, Object msg) throws Exception {
            if (msg instanceof Http3ControlStreamFrame) {
              ...
              switch ((int)controlStreamFrame.type()) {
                case 7:
                  // Go away
                  Runnable runnable = goAwayHandlerRef.get();
                  if (runnable != null) {
                    runnable.run();
                  }
                  break;
    ...
    public class Connection {
      ...
      private final AtomicReference<Runnable> goAwayHandlerRef;

      private Connection(QuicChannel quicChannel, AtomicReference<Runnable> goAwayHandlerRef, InetSocketAddress address) {
        this.channel = quicChannel;
        this.address = address;
        this.goAwayHandlerRef = goAwayHandlerRef;
      }

      public Connection goAwayHandler(Runnable handler) {
