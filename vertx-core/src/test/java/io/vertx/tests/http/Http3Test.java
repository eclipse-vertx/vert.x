...
public class Http3Test extends HttpTest {
  ...
  @Test
  public void testClientMakeRequestHttp3WithSSLWithoutAlpn() throws Exception {
    client.close();
  ...
  public void testInitialMaxConcurrentStreamZero() throws Exception {
    ...
    server.connectionHandler(conn -> {
      vertx.setTimer(500, id -> {
        conn.updateHttpSettings(new HttpSettings(new Http3Settings()));
      });
    });
    startServer(testAddress);
    client.close();
  ...
  public void testClearTextUpgradeWithBody() throws Exception {
    ...
    startServer(testAddress);
    client.close();
    client = vertx.createHttpClient(new HttpClientOptions().setProtocolVersion(HttpVersion.HTTP_3));
    client = vertx.httpClientBuilder()
      .with(new HttpClientOptions().setProtocolVersion(HttpVersion.HTTP_3))
      .withConnectHandler(conn -> {
  ...
  /**
   * Test that socket close (without an HTTP/3 go away frame) removes the connection from the pool
    ...
    server.requestHandler(req -> {
      ...
      switch (req.path()) {
        ...
        case "/2":
          ...
          // Socket close without HTTP/3 go away
  ...
  private void testUnsupportedAlpnVersion(SSLEngineOptions engine, boolean accept) throws Exception {
    ...
    server = vertx.createHttpServer(createBaseServerOptions()
      ...
      .setAlpnVersions(Collections.singletonList(HttpVersion.HTTP_3))
    );
