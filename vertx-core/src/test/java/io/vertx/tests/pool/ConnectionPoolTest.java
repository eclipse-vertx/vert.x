...
public class ConnectionPoolTest extends VertxTestBase {
  ...
  public void testConnect() {
    ...
    pool
      .acquire(context, 0, TestUtils.onSuccess2(lease -> {
      Assert.assertSame(expected, lease.get());
      Assert.assertEquals(0, pool.requests());
      testComplete();
    }));
    Assert.assertEquals(1, pool.requests());
    ConnectionRequest request = mgr.assertRequest();
    Assert.assertSame(context.nettyEventLoop(), request.context.nettyEventLoop());
  ...
  public void testRecycleRemovedConnection() throws Exception {
    ...
    CountDownLatch latch = new CountDownLatch(1);
    fut.onComplete(TestUtils.onSuccess(lease -> {
    ...
    pool.acquire(context, 0, TestUtils.onSuccess2(lease -> {
        Assert.assertSame(expected2, lease.get());
        testComplete();
      }));
  ...
  public void testConcurrency() throws Exception {
    ...
    CountDownLatch latch = new CountDownLatch(1);
    pool
      .acquire(context, 0, TestUtils.onSuccess2(conn -> {
        latch.countDown();
      }));
    ConnectionRequest request = mgr.assertRequest();
    ...
    TestUtils.awaitLatch(latch);
    pool.acquire(context, 0, TestUtils.onSuccess2(lease -> {
        Assert.assertSame(lease.get(), expected);
        testComplete();
      }));
    await();
  }
  ...
  public void testIncreaseConcurrency() throws Exception {
    ...
    pool.acquire(ctx, 0, TestUtils.onSuccess2(lease -> l1.countDown()));
    CountDownLatch l2 = new CountDownLatch(1);
    pool
      .acquire(ctx, 0, TestUtils.onSuccess2(lease -> {
        l2.countDown();
      }));
    CountDownLatch l3 = new CountDownLatch(1);
    pool.acquire(ctx, 0, TestUtils.onSuccess2(lease -> l3.countDown()));
    ConnectionRequest request = mgr.assertRequest();
    request.connect(conn1, 0);
    TestUtils.awaitLatch(l1);
    Assert.assertEquals(1, l2.getCount());
    request.listener.onConcurrencyChange(2);
    TestUtils.awaitLatch(l2);
    request.listener.onConcurrencyChange(3);
    TestUtils.awaitLatch(l3);
  }
  ...
  public void testSatisfyPendingWaitersWithExtraConcurrency() throws Exception {
    ...
    pool
      .acquire(context, 0, TestUtils.onSuccess2(lease -> {
        Assert.assertSame(lease.get(), expected);
        Assert.assertEquals(0, seq.getAndIncrement());
      }));
    pool
      .acquire(context, 0, TestUtils.onSuccess2(lease -> {
        Assert.assertSame(lease.get(), expected);
        Assert.assertEquals(1, seq.getAndIncrement());
        testComplete();
      }));
    ConnectionRequest request = mgr.assertRequest();
    request.concurrency(2).connect(expected, 0);
  ...
  public void testEmptyConcurrency() {
    ...
    pool
      .acquire(context, 0, TestUtils.onSuccess2(lease -> {
        Assert.assertSame(lease.get(), expected);
        Assert.assertEquals(2, seq.getAndIncrement());
        testComplete();
      }));
    ConnectionRequest request = mgr.assertRequest();
  ...
  public void testWaiter() throws Exception {
    ...
    pool.acquire(ctx1, 0, TestUtils.onSuccess2(latch::complete));
    ConnectionRequest request = mgr.assertRequest();
    request.connect(expected, 0);
    ...
    pool
      .acquire(ctx2, 0, TestUtils.onSuccess2(lease2 -> {
        Assert.assertSame(ctx1.nettyEventLoop(), ((ContextInternal) Vertx.currentContext()).nettyEventLoop());
  ...
  public void testRemoveSingleConnection() throws Exception {
    ...
    Assert.assertEquals(0, pool.capacity());
  }
  ...
  public void testRemoveFirstConnection() throws Exception {
    ...
    pool.acquire(ctx, 0, TestUtils.onSuccess2(latch1::complete));
    Connection conn2 = new Connection();
    CompletableFuture<Lease<Connection>> latch2 = new CompletableFuture<>();
    pool.acquire(ctx, 0, TestUtils.onSuccess2(latch2::complete));
    ConnectionRequest request1 = mgr.assertRequest();
    ...
    Assert.assertEquals(1, pool.capacity());
  }
  ...
  public void testRemoveSingleConnectionWithWaiter() throws Exception {
    ...
    CompletableFuture<Lease<Connection>> latch = new CompletableFuture<>();
    pool
      .acquire(ctx1, 0, TestUtils.onSuccess2(latch::complete));
    ConnectionRequest request1 = mgr.assertRequest();
    request1.connect(connection1, 0);
    Lease<Connection> lease1 = latch.get(10, TimeUnit.SECONDS);
    Assert.assertSame(connection1, lease1.get());
    AtomicBoolean evicted = new AtomicBoolean();
    Connection conn2 = new Connection();
    ContextInternal ctx2 = vertx.createEventLoopContext();
    pool
      .acquire(ctx2, 0, TestUtils.onSuccess2(lease2 -> {
        Assert.assertSame(ctx2.nettyEventLoop(), ((ContextInternal) Vertx.currentContext()).nettyEventLoop());
        Assert.assertTrue(evicted.get());
        Assert.assertSame(conn2, lease2.get());
        testComplete();
      }));
    Assert.assertEquals(1, pool.waiters());
    evicted.set(true);
  ...
  public void testConnectFailureWithPendingWaiter() throws Exception {
    ...
    ContextInternal ctx1 = vertx.createEventLoopContext();
    pool
      .acquire(ctx1, 0, TestUtils.onFailure2(cause -> {
        Assert.assertSame(failure, cause);
        Assert.assertEquals(1, pool.requests());
        latch.countDown();
      }));
    ContextInternal ctx2 = vertx.createEventLoopContext();
    pool.acquire(ctx2, 1, TestUtils.onSuccess2(lease -> {
        Assert.assertSame(expected, lease.get());
        testComplete();
      }));
    ConnectionRequest request1 = mgr.assertRequest();
    Assert.assertEquals(2, pool.capacity());
  ...
  public void testExpireFirst() throws Exception {
    Assert.assertEquals(Arrays.asList(0), testExpire(1, 10, 0));
    Assert.assertEquals(Arrays.asList(0), testExpire(2, 10, 0));
    Assert.assertEquals(Arrays.asList(0), testExpire(3, 10, 0));
  }

  @Test
  public void testExpireLast() throws Exception {
    Assert.assertEquals(Arrays.asList(0), testExpire(1, 10, 0));
    Assert.assertEquals(Arrays.asList(1), testExpire(2, 10, 1));
    Assert.assertEquals(Arrays.asList(2), testExpire(3, 10, 2));
  }

  @Test
  public void testExpireMiddle() throws Exception {
    Assert.assertEquals(Arrays.asList(1), testExpire(3, 10, 1));
  }

  @Test
  public void testExpireSome() throws Exception {
    Assert.assertEquals(Arrays.asList(2, 1), testExpire(3, 10, 1, 2));
    Assert.assertEquals(Arrays.asList(2, 1, 0), testExpire(3, 10, 0, 1, 2));
    Assert.assertEquals(Arrays.asList(1, 0), testExpire(3, 10, 0, 1));
  }
  ...
  public void testRemoveEvicted() throws Exception {
    ...
    pool.evict(c -> c == conn, TestUtils.onSuccess2(l -> latch2.countDown()));
  ...
  public void testSynchronousEviction() throws Exception {
    ...
    pool.evict(candidate -> {
      Assert.assertSame(candidate, conn1);
      pool
        .acquire(ctx, 0, TestUtils.onSuccess2(lease -> {
          Connection c2 = lease.get();
          Assert.assertSame(conn2, c2);
          latch3.countDown();
        }));
      return true;
    }, TestUtils.onSuccess2(list -> {
      latch2.countDown();
    }));
    TestUtils.awaitLatch(latch2);
  ...
  public void testConnectionInProgressShouldNotBeEvicted() {
    ...
    pool
      .evict(c -> {
        Assert.fail();
        return false;
      }, TestUtils.onSuccess2(v -> testComplete()));
    await();
  }
  ...
  public void testMaxWaiters() {
    ...
    for (int i = 0;i < (5);i++) {
      pool
        .acquire(ctx, 0, (res, err) -> Assert.fail());
    }
    pool
      .acquire(ctx, 0, TestUtils.onFailure2(err -> {
        Assert.assertTrue(err instanceof ConnectionPoolTooBusyException);
        testComplete();
      }));
    await();
  }
  ...
  public void testHeterogeneousSizes() throws Exception {
    ...
    for (int i = 0;i < 5;i++) {
      pool.acquire(ctx, 0, TestUtils.onSuccess2(lease -> latch.countDown()));
      Connection conn = new Connection();
      mgr.assertRequest().connect(conn, 0);
    }
    TestUtils.awaitLatch(latch);
    Assert.assertEquals(10, pool.capacity());
    pool
      .acquire(ctx, 1, TestUtils.onSuccess2(lease -> {
  ...
  public void testClose() throws Exception {
    ...
    waitFor(3);
    pool.acquire(ctx, 0, TestUtils.onFailure2(err -> complete()));
    pool.acquire(ctx, 0, TestUtils.onFailure2(err -> complete()));
    mgr.assertRequest().connect(conn1, 0);
    mgr.assertRequest();
    pool
      .close(TestUtils.onSuccess2(lst -> {
  ...
  public void testCloseTwice() throws Exception {
    ...
    pool
      .close(TestUtils.onSuccess2(lst -> {
        ...
        pool
          .close(TestUtils.onFailure2(err -> {
            isReentrant.set(inCallback.get());
            latch.countDown();
          }));
      }));
    TestUtils.awaitLatch(latch);
    Assert.assertFalse(isReentrant.get());
  }
  ...
  public void testUseAfterClose() throws Exception {
    ...
    TestUtils.awaitLatch(latch);
    pool.evict(c -> true, TestUtils.onFailure2(err -> complete()));
    pool.acquire(ctx, 0, TestUtils.onFailure2(err -> complete()));
    pool.cancel(waiter, TestUtils.onFailure2(err -> complete()));
  ...
  public void testAcquireClosedConnection() throws Exception {
    ...
    ConnectionPool<Connection> pool = ConnectionPool.pool(mgr, new int[] { 1 });
    ContextInternal context = vertx.createEventLoopContext();
    pool.acquire(context, 0, TestUtils.onSuccess2(Lease::recycle));
    Connection expected = new Connection();
    ConnectionRequest request = mgr.assertRequest();
    request.connect(expected, 0);
    CountDownLatch latch1 = new CountDownLatch(1);
    ...
    pool
      .acquire(context, 0, TestUtils.onSuccess2(lease -> {
        ...
        Assert.assertNotNull(lease.get());
  ...
  private void testConnectResultAfterClose(boolean success) {
    ...
    pool
      .acquire(ctx, 0, (res, err) -> {
        Assert.assertEquals(0, acquired.getAndIncrement());
      });
    Assert.assertEquals(1, pool.size());
    ConnectionRequest request = mgr.assertRequest();
    ...
    Assert.assertTrue(closeResult.isComplete());
    List<Future<Connection>> connections = closeResult.result();
    Assert.assertEquals(1, connections.size());
    Assert.assertEquals(success, connections.get(0).succeeded());
    Assert.assertEquals(0, pool.size());
    if (success) {
      Assert.assertEquals(expected, connections.get(0).result());
    } else {
      Assert.assertEquals(cause, connections.get(0).cause());
    }
  ...
  public void testCancelQueuedWaiters() throws Exception {
    ...
    pool.acquire(context, new PoolWaiter.Listener<>() {
      ...
      }, 0, (res, err) -> Assert.fail());
    PoolWaiter<Connection> waiter = w.get(10, TimeUnit.SECONDS);
    pool
      .cancel(waiter, TestUtils.onSuccess2(removed1 -> {
        Assert.assertTrue(removed1);
        Assert.assertEquals(0, pool.waiters());
        pool
          .cancel(waiter, TestUtils.onSuccess2(removed2 -> {
  ...
  public void testCancelWaiterBeforeConnection(boolean success, int extra) throws Exception {
    ...
    for (int i = 0; i < extra; i++) {
      pool.acquire(context, new PoolWaiter.Listener<>() {
        ...
        }, 0, TestUtils.onSuccess2(conn -> {
          conn.recycle();
          recycledLatch.countDown();
        }));
    }
    TestUtils.awaitLatch(enqueuedLatch);
    ConnectionRequest request = mgr.assertRequest();
    CountDownLatch latch = new CountDownLatch(1);
    pool
      .cancel(waiterLatch.get(10, TimeUnit.SECONDS), TestUtils.onSuccess2(removed -> {
    ...
    TestUtils.awaitLatch(recycledLatch);
  ...
  public void testCancelWaiterAfterConnectionSuccess(boolean success) throws Exception {
    ...
    TestUtils.awaitLatch(latch);
    pool
      .cancel(w.get(10, TimeUnit.SECONDS), TestUtils.onSuccess2(removed -> {
  ...
  public void testConnectionSelector() throws Exception {
    ...
    pool.connectionSelector((waiter, list) -> {
      ...
      Assert.assertEquals(1, pooled.available());
      Assert.assertEquals(1, pooled.concurrency());
      Assert.assertSame(conn1, pooled.get());
      Assert.assertSame(context.nettyEventLoop(), pooled.context().nettyEventLoop());
      Assert.assertSame(context, waiter.context());
      return pooled;
    });
    pool
      .acquire(context, 0, TestUtils.onSuccess2(lease -> {
  ...
  public void testDefaultSelector() throws Exception {
    ...
    CountDownLatch latch1 = new CountDownLatch(1);
    pool
      .acquire(context1, 0, TestUtils.onSuccess2(lease -> {
        lease.recycle();
        latch1.countDown();
      }));
    Connection expected = new Connection();
    ...
    pool
      .acquire(context1, 0, TestUtils.onSuccess2(lease -> {
        Assert.assertEquals(expected, lease.get());
        lease.recycle();
        latch2.countDown();
      }));
    TestUtils.awaitLatch(latch2);
    ...
    ContextInternal context2 = vertx
      ...
      .build();
    pool
      .acquire(context2, 0, TestUtils.onSuccess2(lease -> {
        Assert.assertEquals(expected, lease.get());
        lease.recycle();
        latch3.countDown();
      }));
  ...
  public void testDefaultContextProviderUnwrap() {
    ...
    ConnectionPool<Connection> pool = ConnectionPool.pool(mgr, new int[] { 10 }, 10);
    pool
      .acquire(context.duplicate(), 0, TestUtils.onSuccess2(lease -> {
      }));
    Assert.assertEquals(1, pool.requests());
    ConnectionRequest request = mgr.assertRequest();
    Assert.assertSame(context.nettyEventLoop(), request.context.nettyEventLoop());
  ...
  public void testPostTasksTrampoline() throws Exception {
    ...
    ConnectionPool<Connection> pool = ConnectionPool.pool(new PoolConnector<Connection>() {
      ...
      public Future<ConnectResult<Connection>> connect(ContextInternal context, Listener listener) {
        Assert.assertEquals(0, reentrancy++);
        try {
          int val = count[0]++;
          if (val == 0) {
    ...
    TestUtils.awaitLatch(latch);
    Assert.assertEquals(1 + numAcquires, count[0]);
  ...
  public void testConcurrentPostTasksTrampoline() throws Exception {
    ...
    ref1.set(pool1);
    ref2.set(pool2);
    pool1.acquire(ctx, 0, TestUtils.onFailure2(err -> res.add(0)));
    TestUtils.awaitLatch(latch);
