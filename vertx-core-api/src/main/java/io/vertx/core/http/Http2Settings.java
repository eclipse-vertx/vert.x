...
public class Http2Settings {
  ...
  public Http2Settings setHeaderTableSize(long headerTableSize) {
    if (headerTableSize < Http2CodecUtil.MIN_HEADER_TABLE_SIZE) {
      throw new IllegalArgumentException("headerTableSize must be >= " + Http2CodecUtil.MIN_HEADER_TABLE_SIZE);
    }
    if (headerTableSize > Http2CodecUtil.MAX_HEADER_TABLE_SIZE) {
      throw new IllegalArgumentException("headerTableSize must be <= " + Http2CodecUtil.MAX_HEADER_TABLE_SIZE);
    }
  ...
  public Http2Settings setInitialWindowSize(int initialWindowSize) {
    if (initialWindowSize < Http2CodecUtil.MIN_INITIAL_WINDOW_SIZE) {
      throw new IllegalArgumentException("initialWindowSize must be >= " + Http2CodecUtil.MIN_INITIAL_WINDOW_SIZE);
    }
  ...
  public Http2Settings setMaxFrameSize(int maxFrameSize) {
    if (maxFrameSize < Http2CodecUtil.MAX_FRAME_SIZE_LOWER_BOUND) {
      throw new IllegalArgumentException("maxFrameSize must be >= " + Http2CodecUtil.MAX_FRAME_SIZE_LOWER_BOUND);
    }
    if (maxFrameSize > Http2CodecUtil.MAX_FRAME_SIZE_UPPER_BOUND) {
      throw new IllegalArgumentException("maxFrameSize must be <= " + Http2CodecUtil.MAX_FRAME_SIZE_UPPER_BOUND);
    }
  ...
  public Http2Settings setMaxHeaderListSize(long maxHeaderListSize) {
    if (maxHeaderListSize < Http2CodecUtil.MIN_HEADER_LIST_SIZE) {
      throw new IllegalArgumentException("maxHeaderListSize must be >= " + Http2CodecUtil.MIN_HEADER_LIST_SIZE);
    }
  ...
  public Http2Settings set(int id, long value) {
    if (id < 0 || id > 0xFFFF) {
      throw new IllegalArgumentException("Setting id must me an unsigned 16-bit value");
    }
    if (value < 0L || value > 0xFFFFFFFFL) {
      throw new IllegalArgumentException("Setting value must me an unsigned 32-bit value");
    }
    switch (id) {
      case 1:
      ...
      case 2:
        if (value != 0 && value != 1) {
          throw new IllegalArgumentException("enablePush must be 0 or 1");
        }
      ...
      case 6:
        if (value > Integer.MAX_VALUE) {
          throw new IllegalArgumentException("maxHeaderListSize must be <= " + Integer.MAX_VALUE);
        }
